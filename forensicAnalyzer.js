...
// Requiere evidencia de código real (una acción tipada /S /JavaScript, o un
// /JS seguido de un string/stream de contenido) — NO solo la presencia aislada
// de la cadena "/JS" u "/OpenAction", que muchos generadores usan de forma
// inocua (p. ej. para fijar el zoom o la página inicial al abrir el archivo).
function detectarJavaScript(texto) {
  const accionTipadaJS = /\/S\s*\/JavaScript\b/.test(texto);
  const codigoJSAdjunto = /\/JS\s*[\(\<]/.test(texto);
  return accionTipadaJS || codigoJSAdjunto;
}
...
function analizarPDF(buffer, opts = {}) {
  ...
  if (!software.esLegitimo && (software.productor || software.creador)) {
    ...
    anomalias.push({
      ...
      severidad: 'media',
      detalle: 'No se pudo identificar el software que generó el documento (sin /Producer ni /Creator). Es común en portales oficiales que remueven esta metadata a propósito; por sí solo no es señal fuerte de manipulación.',
    });
  }
