...
const { verificarLimite, incrementarUso, obtenerUso, establecerPlan, obtenerLimite, LIMITES } = require('./lib/planes');
...
const { crearApiKey, listarApiKeys, revocarApiKey, obtenerApiKey, actualizarLimiteApiKey, actualizarWebhookApiKey } = require('./lib/apiKeys');
...
app.post('/api/admin/limite', verificarAdmin, async (req, res) => {
  const { api_key, limite_mensual } = req.body || {};
  if (!api_key) return res.status(400).json({ error: 'Falta "api_key" en el body.' });
  try {
    const actualizado = await actualizarLimiteApiKey(api_key, limite_mensual);
    if (!actualizado) return res.status(404).json({ error: 'Esa API key no tiene un registro dinámico.' });
    res.json({ ok: true, api_key, limite_mensual: limite_mensual || null });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
app.post('/api/admin/keys', verificarAdmin, async (req, res) => {
  const { nombre_cliente, plan, limite_mensual } = req.body || {};
  try {
    const nueva = await crearApiKey({ nombreCliente: nombre_cliente, plan: plan || 'free', limiteMensual: limite_mensual });
...
app.get('/api/admin/keys', verificarAdmin, async (req, res) => {
  ...
  const keysConUso = await Promise.all(keys.map(async k => {
    ...
    const limite = await obtenerLimite(k.clave, k.plan);
    return { ...k, verificaciones_este_mes: uso, limite_mensual: limite, limite_personalizado: k.limiteMensual !== undefined };
  }));
